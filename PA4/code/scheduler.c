...
static void suspend_worker(thread_info_t *info)
{
	...
	/* update time thread is suspended*/
...
void setup_sig_handlers() {
	...
	if (sigemptyset(&newsigmask) == -1 || sigaddset(&newsigmask, SIGALRM) == -1)
	{
	...
	if (sigemptyset(&newsigmask) == -1 || sigaddset(&newsigmask, SIGTERM) == -1)
	{
	...
	if (sigemptyset(&newsigmask) == -1 || sigaddset(&newsigmask, SIGUSR1) == -1)
	{
...
static void *scheduler_run(void *unused)
{
	...
	time_val.it_value.tv_sec = QUANTUM;
	time_val.it_value.tv_nsec = 0;
	time_val.it_interval.tv_sec = QUANTUM;
	time_val.it_interval.tv_nsec = 0;

    if (timer_settime(timer, 0, &time_val, NULL) == -1){
