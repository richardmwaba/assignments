...
{   
	...
	if(sigemptyset(&set) == -1 || sigaddset(&set,SIGUSR2) == -1) {
		perror("Failure to create signal mask for SIGUSR2.");
		pthread_exit(NULL);
	}
...
void *start_worker(void *arg)
{
	...
	if(sigemptyset(&sigset) == -1 || sigaddset(&sigset,SIGALRM) == -1 || sigaddset(&sigset,SIGUSR2) == -1) {
		perror("Failure to initialize blocking signal masks SIGALRM and SIGUSR2.");
	}
	if(pthread_sigmask(SIG_BLOCK,&sigset,NULL) == -1) {
		perror("Failure to block signals SIGALRM and SIGUSR2.");
	}

	/* TODO: Unblock SIGUSR1 and SIGTERM. */
	if(sigemptyset(&sigset) == -1 || sigaddset(&sigset,SIGTERM) == -1 || sigaddset(&sigset,SIGUSR1) == -1)  {
		perror("Failure to initialize unblocking signal mask for signal SIGTERM and SIGUSR1.");
	}
	if(pthread_sigmask(SIG_UNBLOCK,&sigset,NULL) == -1) {
		perror("Failure to unblock signals SIGTERM and SIGUSR1.");
	}
